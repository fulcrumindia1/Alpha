...
# PART 10: Concurrency control — limit concurrent Chromium subprocesses to 2 to protect KVM2 RAM/CPU
_PDF_CONCURRENCY_SEMAPHORE = threading.BoundedSemaphore(2)
...
def _convert_html_to_pdf_playwright(html_content: str) -> Optional[bytes]:
    ...
    Protected by concurrency semaphore and hard timeout.
    """
    # Attempt to acquire a slot without blocking forever
    acquired = _PDF_CONCURRENCY_SEMAPHORE.acquire(blocking=True, timeout=15)
    if not acquired:
        print("[PDFGenerator] WARN: High concurrency; PDF slot busy, falling back to ReportLab")
        return None
    ...
    try:
        ...
        print("[PDFGenerator] Playwright subprocess timed out after 40s; cleanly falling back to ReportLab")
        return None
    except Exception as e:
        print(f"[PDFGenerator] Playwright subprocess engine failed: {e}")
        return None
    finally:
        _PDF_CONCURRENCY_SEMAPHORE.release()
...
def _generate_emergency_pdf(full_name: str, data: dict) -> bytes:
    """
    Emergency absolute fallback: generates a valid, styled 1-page summary using ReportLab canvas.
    Guarantees 0 dependencies on Playwright or HTML parser and NEVER produces a blank page.
    """
    try:
        import io
        from reportlab.lib.pagesizes import A4
        ...
        c.drawString(30, height - 38, "FULCRUM-INDIA -- FOUNDER DOSSIER SUMMARY")

        # Founder Details
        c.setFillColor(HexColor("#0F172A"))
        c.setFont("Helvetica-Bold", 14)
        c.drawString(30, height - 90, f"Founder: {full_name}")
        ...
        lines = [
            f"Email: {prof.get('email', 'N/A')}",
            f"Phone: {prof.get('phone', 'N/A')}",
            f"District: {prof.get('district', 'Tamil Nadu')}",
            f"Business: {biz.get('business_name', 'N/A')} ({biz.get('category', 'General')})",
            f"Business Stage: {biz.get('business_stage', 'Early')}",
            f"Turnover: Rs. {biz.get('annual_turnover', 0):,}",
            f"Active Schemes Tracked: {len(data.get('schemes', []))}",
            f"Journey Milestones Logged: {len(data.get('journey', []))}",
            f"Open Queries / Help Requests: {len(data.get('help_requests', []))}",
            "",
            "Generated automatically by FULCRUM-INDIA Emergency Engine.",
        ]
        ...
        print(f"[PDFGenerator] Emergency canvas generator failed: {e}")
        content = f"BT /F1 12 Tf 50 750 Td (FULCRUM-INDIA Founder Dossier: {full_name}) Tj 0 -20 Td (Generated by Emergency Fallback) Tj ET"
...
def generate_aspirant_dossier_pdf(aspirant_id: str) -> bytes:
    ...
    # Last resort: Clean emergency text dossier (never returns a blank page!)
    print(f"[PDFGenerator] FAIL Both engines failed for '{full_name}', generating emergency text dossier")
    return _generate_emergency_pdf(full_name, data)
